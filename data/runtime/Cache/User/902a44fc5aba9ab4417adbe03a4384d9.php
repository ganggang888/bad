...
<div class="weui-navbar" style="position:fixed; top:44px; left:0; right:0; height:44px; background:#fff;">
      ...
      <a class="weui-navbar__item proinfo-tab-tit font-14 weui-bar__item--on" href="<?php echo leuu('User/Center/fx_log');?>">返现收益记录</a>
      <a class="weui-navbar__item proinfo-tab-tit font-14 weui-bar__item" href="<?php echo leuu('User/Other/lcsy_history');?>">理财收益记录</a>
  ...
  <div class="weui-cells wy-news-list" style="margin-top:43px">
     ...
     <?php if(is_array($result)): foreach($result as $key=>$vo): ?><a class="weui-cell weui-cell">
      <div class="weui-cell__bd">
        <p>你的<?php if($vo['level'] == 1): ?>下级 <?php elseif($vo['level'] == 2): ?>下下级<?php endif; ?>购买商品所得：<?php echo ($vo["score"]); ?></p>
        <p>
        <?php echo getSomeMessage('users',$vo['pid'],'mobile');?>
        <?php if($vo['status'] == 1): ?>推荐奖
        <?php elseif($vo['status'] == 2): ?>
        抽佣
        <?php elseif($vo['status'] == 3): ?>
        见点奖<?php endif; ?> 
        <?php if($vo['times'] == 1): ?>(首单下单)<?php endif; ?>
        </p>
      </div>
