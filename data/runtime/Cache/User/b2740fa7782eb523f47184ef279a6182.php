...
<div class="weui-navbar" style="position:fixed; top:44px; left:0; right:0; height:44px; background:#fff;">
     <a class="weui-navbar__item proinfo-tab-tit font-14 weui-bar__item<?php if($_GET['status'] == 0): ?>--on<?php endif; ?>" href="<?php echo leuu('User/Other/tixian_history');?>">全部</a>
      
      <a class="weui-navbar__item proinfo-tab-tit font-14 weui-bar__item<?php if($_GET['status'] == 1): ?>--on<?php endif; ?>" href="<?php echo leuu('User/Other/tixian_history',array('status'=>1));?>">处理中</a>
      <a class="weui-navbar__item proinfo-tab-tit font-14 weui-bar__item<?php if($_GET['status'] == 2): ?>--on<?php endif; ?>" href="<?php echo leuu('User/Other/tixian_history',array('status'=>2));?>">已成功</a>
      <a class="weui-navbar__item proinfo-tab-tit font-14 weui-bar__item<?php if($_GET['status'] == 3): ?>--on<?php endif; ?>" href="<?php echo leuu('User/Other/tixian_history',array('status'=>3));?>">已驳回</a>
  ...
  <div class="weui-cells wy-news-list" style="padding-top:30px">
     ...
     <?php if(is_array($result)): foreach($result as $key=>$vo): ?><a class="weui-cell weui-cell">
      <div class="weui-cell__bd">
        <p>提现<?php echo ($vo["money"]); ?> 元</p>
        <p>银行卡：<?php echo getSomeMessage('bank',$vo['bank_id'],'name');?>-<?php echo getSomeMessage('bank',$vo['bank_id'],'bank_name');?></p>
        <p>状态：<?php if($vo['status'] == 1): ?>处理中
         <?php elseif($vo['status'] == 2): ?>
         成功
         <?php elseif($vo['status'] == 3): ?>
         驳回<?php endif; ?>&nbsp;&nbsp;
        <?php if($vo['type'] == 1): ?>返现积分提现
