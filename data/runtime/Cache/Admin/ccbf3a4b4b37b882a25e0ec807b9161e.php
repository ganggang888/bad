		...
		<form method="post" class="form-horizontal js-ajax-form" action="">
            ...
            <div class="control-group">
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" value="<?php echo ($info["name"]); ?>" required name="name" placeholder="请输入商品名称">
						<span class="form-required">*</span>
					</div>
				</div>
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" required name="score" placeholder="请输入兑换积分" value="<?php echo ($info["score"]); ?>">
						<span class="form-required">*</span>
					</div>
				</div>
                ...
                <div class="control-group">
					...
					<div class="controls">
                        ...
                        <option value="1" <?php if($info['huangjin'] == 1): ?>selected<?php endif; ?>>是</option>
                        <option value="0" <?php if($info['huangjin'] == 0): ?>selected<?php endif; ?>>否</option>
                        </select>
						<span class="form-required">*</span>
					</div>
				</div>
                ...
                <div class="control-group">
					...
					<div class="controls">
						<select name="status">
                        <option value="1" <?php if($info['status'] == 1): ?>selected<?php endif; ?>>上架</option>
                        <option value="0" <?php if($info['status'] == 0): ?>selected<?php endif; ?>>下架</option>
                        </select>
						<span class="form-required">*</span>
					</div>
				</div>
                ...
                <div class="control-group">
					...
					<div class="controls">
						<textarea name="label" rows="5"><?php echo ($info["label"]); ?></textarea>
				...
				<div class="control-group">
					...
					<div class="controls">
						...
						<ul id="photos" class="pic-list unstyled">
										<?php if(is_array($info['photos_url'])): foreach($info['photos_url'] as $key=>$vo): $img_url=sp_get_image_preview_url($vo); ?>
										<li id="savedimage<?php echo ($key); ?>">
											<input id="photo-<?php echo ($key); ?>" type="hidden" name="photos_url[]" value="<?php echo ($img_url); ?>"> 
											<input id="photo-<?php echo ($key); ?>-name" type="text" name="photos_alt[]" value="<?php echo ($vo); ?>" style="width: 200px;" title="图片名称">
											<img id="photo-<?php echo ($key); ?>-preview" src="<?php echo sp_get_image_preview_url($vo);?>" style="height:36px;width: 36px;" onClick="parent.image_preview_dialog(this.src);">
                ...
                <?php if($attribute): if(is_array($attribute)): foreach($attribute as $keys=>$vos): ?><div class="control-group">
					<label class="control-label">对应属性</label>
					<div class="controls">
						<select class="shuxing" name="attribute[id][]">
                        <option value="0">请选择</option>
                        <?php if(is_array($attributes)): foreach($attributes as $key=>$vo): if(is_array($vo['son'])): foreach($vo['son'] as $k=>$v): ?><option value="<?php echo ($key); ?>-<?php echo ($k); ?>" <?php if($vos['id'] == $key.'-'.$k): ?>selected<?php endif; ?>><?php echo ($vo["name"]); ?>/<?php echo ($v["name"]); ?>/<?php echo ($v["info"]); ?></option><?php endforeach; endif; endforeach; endif; ?>
                        </select>
                        <?php if($keys): ?><span class="form-required"><a href="javascript:;" class="deleteOne">删除</a></span>
                        <?php else: ?>
                        <span class="form-required"><a href="javascript:;" id="addMore">添加更多</a></span><?php endif; ?>
                ...
                <div class="control-group" style="border-bottom:dotted 1px #333">
					...
					<div class="controls about">
						<input type="text" required name="attribute[cost_price][]" value="<?php echo ($vos["cost_price"]); ?>" placeholder="成品价">
                        <input type="text" required name="attribute[selling_price][]" value="<?php echo ($vos["selling_price"]); ?>" placeholder="销售价">
                        <input type="text" required name="attribute[market_value][]" value="<?php echo ($vos["market_value"]); ?>" placeholder="市场价">
                        <input type="text" required name="attribute[unit][]" value="<?php echo ($vos["unit"]); ?>" placeholder="商品单位">
                        <input type="text" required name="attribute[stock][]" value="<?php echo ($vos["stock"]); ?>" placeholder="库存">
                        <input type="text" required name="attribute[name][]" value="<?php echo ($vos["name"]); ?>" placeholder="">
                        <input type="text" required name="attribute[info][]" value="<?php echo ($vos["info"]); ?>" placeholder="">
					</div>
